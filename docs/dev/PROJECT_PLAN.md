...
molecule:
  charge: 0  # 必需字段：分子电荷，直接复制到 SCF 模块的 Charge 关键词
  multiplicity: 1  # 必需字段：自旋多重度（2S+1），直接复制到 SCF 模块的 Spin 关键词
                   # 这两个字段必须显式设置，确保用户清楚知道计算的体系电荷和电子态
                   # 转换器会直接将这两个值复制到 SCF 模块，无论值是多少（包括 charge=0 和 multiplicity=1）
  coordinates:
    # 坐标格式: ATOM X Y Z
...
### Phase 5: AI 和执行模块 ⏳ 待开始
- [ ] AI 模块实现
- [ ] BDFAutotest 集成
- [ ] 结果分析模块

**当前状态**：Phase 2 基本完成，Phase 3 进行中
